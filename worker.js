...
export default {
  async fetch(request, env) {
    ...
    if (action === "calendar_add") {
      const token = request.headers.get("X-Session-Token") || body.token;
      if (!await validateSession(env, token)) return jsonResponse({ error: "Unauthorized" }, 401, origin);
      const { title, date, time, end_time, duration_minutes = 60, description = "" } = body;
      if (!title || !date) return jsonResponse({ error: "Title and date required", success: false }, 400, origin);
      try {
        // Parse any time format to total minutes since midnight
        const parseTime = (t) => {
          if (!t) return null;
        ...
        const startMins = parseTime(time) ?? 9 * 60;
        let endMins;
        if (end_time) {
          endMins = parseTime(end_time);
        }
        // If end_time missing or failed to parse, use duration
        if (endMins === null || endMins === undefined) {
          endMins = startMins + (duration_minutes || 60);
        }
        // If end is same or before start, add duration to start instead
        if (endMins <= startMins) {
          endMins = startMins + (duration_minutes || 60);
        }

        const startISO = toISO(startMins);
        const endISO   = toISO(endMins);
